...
function SearchableSelect({ id, activeId, setActiveId, label, value, onChange, options, disabled }) {
    ...
    return (
        <div ref={wrapperRef} className="flex flex-col gap-2 relative w-full font-circular">
            ...
            <input
                ...
                placeholder={label}
                disabled={disabled}
                className="p-3 disabled:bg-gray-50 disabled: disabled:text-black/50 font-circular h-10 block w-full rounded-sm text-sm leading-5 text-secondary-400 shadow transition placeholder:text-gray-400 focus:outline-none focus:ring-4 disabled:opacity-90 border border-gray-200 focus:border-[#3EC0DD] focus:ring-[#3EC0DD]/10"
            />

            {open && (
                <div
                    className="absolute top-16 left-0 w-full border bg-white rounded shadow-md max-h-[200px] overflow-y-auto z-10
...
export default function SelectDate({ sectionData, handleFieldChange, handlePresentChange, title }) {
